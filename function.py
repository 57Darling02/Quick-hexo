...
class gui:
    ...
    def ImageProcess(self,path,r=False,width = 0,height = 0):# assets/bg1.png
    ...
    def create_scrollable_text_area(self,parent, text):
        ...
        # 创建一个文本框
        text_area = tk.Text(parent, yscrollcommand=scrollbar.set, wrap=tk.WORD,state=tk.NORMAL, autoseparators=True,height=1000)
    ...
    def __init__(self):
        ...
        iconIMG = self.ImageProcess(path="assets/logo.png", r=False)
        root.iconphoto(True, iconIMG)
        screen_width = root.winfo_screenwidth()
        screen_height = root.winfo_screenheight()
        # 设置窗口的位置和大小
        root.geometry('{}x{}+{}+{}'.format(root_width, root_height, int((screen_width - root_width) / 2), int((screen_height - root_height) / 2)))
        ...
        # 构造 navigation_bar
        img = {}
        img["bg"] = (self.ImageProcess("assets/bg2.png",True,76,25))
        img["bigbg"] = (self.ImageProcess("assets/bg.png",True,128,60))
        img['start'] = self.ImageProcess("assets/start.png", True, 20, 12)
        img['close'] = self.ImageProcess("assets/close.png", True, 20, 12)
        ...
        checkLabel = tk.Button(navigation_bar,text="Check",command=lambda :check(),padx=18,compound="center",image=img["bg"],bg=navigation_bar["bg"],bd=0,activebackground=navigation_bar["bg"])
        checkLabel.grid(row=0,column=0,padx=1,pady=5)
        HelpLabel = tk.Button(navigation_bar , text="Help",command=lambda : webbrowser.open(self.help_url),padx=18,compound="center",image=img["bg"],bg=navigation_bar["bg"],bd=0,activebackground=navigation_bar["bg"])
        HelpLabel.grid(row=0,column=1,padx=1,pady=5)
        AuthorLabel = tk.Button(navigation_bar, text="Author", command=lambda: webbrowser.open(self.Author_URI),padx=18,compound="center",image=img["bg"],bg=navigation_bar["bg"],bd=0,activebackground=navigation_bar["bg"])
        ...
        settingLabel.bind("<Button-1>",lambda event:[setting_menu.tk_popup(settingLabel.winfo_rootx()-10,settingLabel.winfo_rooty()+30)])
        ...
        consoleBTN = hexofuc.HoverButton(navigation_bottom_bar,bg=navigation_bottom_bar["bg"],bd=0,
        ...
        def create_buttons(config):
            ...
            for key, value in config.items():
                ...
                button = tk.Button(btnframe,image=img["bigbg"],bd=0,bg=mainframe["bg"],compound="center",
                                   ...
                                   text=key, command=lambda v=value: [self.hexo.domore(v),self.ui.show_popup('正在执行命令：\n'+str(v),False,False)])
        ...
        btn2 = tk.Button(btnframe, image=img["bigbg"], bd=0, bg=mainframe["bg"], compound="center", text='new post',padx=10,pady=20,
